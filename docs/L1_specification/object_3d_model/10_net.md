...
## 4.2 底面選択と展開フロー（学習向け）
展開の学習効果を高めるため、**底面（rootFaceId）をユーザーが選択**できるUIフローを採用する。
展開の順序とカメラワークは、この底面選択を起点に決定する。

1. 展開ボタンを押すと「底面選択モード」に入る  
2. 面ホバーで強調表示、クリックで底面を確定  
3. 底面を固定し、展開開始前に斜め上の視点へ移動  
4. ヒンジが見える角度を保ちながら面が順に展開  
5. 展開完了時にカメラが真上へ遷移  

補足：
- 「真上」は底面の法線方向と一致する視点を指す（Net 平面の俯瞰）
- rootFaceId が指定されない場合は構造的に決定してよいが、UI選択を優先する
...
- **90°固定の回転は立方体に限った近似であり、一般多面体では採用しない**
