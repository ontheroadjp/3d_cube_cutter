...
## 10. 展開木（Face Adjacency Graph）
切断前/切断後の分岐ではなく、**現在存在する面集合から隣接グラフを構築し、そこから展開木を1本決める**。

### 10.1 前処理
- 面の頂点は `vertexIds`（SnapPointID）で統一し、順序は CCW を保つ
- 共有辺は `vertexIds` の組を正規化して同定する
- 同一平面で分割された面（分割面）はヒンジ角 0 の隣接として扱う

### 10.2 隣接グラフの構築
- ノード: 面（Face）
- エッジ: 共有辺（2頂点のID組）
- 切断面を経由する隣接も通常のエッジとして扱う

### 10.3 展開木の選択
- グラフから 1 本のスパニングツリーを選ぶ
- 初期は「元の面同士の隣接を優先」「切断面経由は低優先」などの重み付けを採用
- 切断面経由が必須な場合は橋として採用する

### 10.4 展開実行
- 展開中は分類/判定を行わず、展開木に従って機械的に回転する
- 回転軸は共有辺（vertexIds から resolver で座標解決）
