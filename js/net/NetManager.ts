...
export class NetManager {
    ...
    generateNetPlan(solid: SolidSSOT, rootFaceId?: FaceID): NetPlan {
        ...
        while (queue.length > 0) {
            ...
            const parentFace = solid.faces[parentId];
            const parentEdges = this.getFaceEdges(parentId, solid);

            // Find adjacent faces that haven't been visited
    ...
    update(cutSegments: any[], solid: any, resolver: GeometryResolver | null = null) {
        ...
        cutSegments.forEach(segment => {
            let faceId = segment.faceId || (segment.faceIds && segment.faceIds[0]);
            if (!faceId) {
                // Try to resolve faceId from start/end IDs if possible
                // (Simplified for now)
            }
            if (!faceId) return;
